...
case class GlutenDeleteCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    recordDeltaOperation(deltaLog, "delta.dml.delete") {
      ...
      val deletedRowsByFile = findDeletedRowsByFile(sparkSession, filesToModify, condition)
  ...
  private def findDeletedRowsByFile(
    ...
    condition match {
      ...
      case Some(expr) =>
        ...
        val rowFinderConfig = buildRowIndexFinderConfig(sparkSession)
        val rowFinderParallelism = buildRowIndexFinderParallelism(sparkSession, absolutePaths.size)

        try {
          // Delta snapshot files generally share a compatible physical schema. Serialize once so
          // native row discovery can fan out across files without redoing Spark-side work.
          val sharedSchema = sparkSession.read.parquet(absolutePaths.head._2).schema
          ...
          case NonFatal(e) =>
            logWarning(
              s"Shared-schema native row discovery failed for DELETE; " +
  ...
  private def buildRowIndexFinderConfig(
      sparkSession: SparkSession): VeloxRowIndexFinderJni.Config = {
    VeloxRowIndexFinderJni.Config(
      batchSize =
        sparkSession.conf.get("spark.gluten.delta.rowIndexFinder.batchSize", "10000").toInt,
      numThreads = sparkSession.conf.get("spark.gluten.delta.rowIndexFinder.numThreads", "1").toInt,
      memoryLimitMB =
