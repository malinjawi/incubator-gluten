...
object DeltaPostTransformRules {
  ...
  val plainDeltaFallbackRule: Rule[SparkPlan] = (plan: SparkPlan) =>
    plan.transformUp {
      case scan: DeltaScanTransformer
          if isDeltaLikeFileFormat(scan.relation.fileFormat) &&
            !hasDeletionVectorMarkers(scan.output, scan.requiredSchema) =>
        FallbackTags.add(scan, "fallback plain Delta scan without DV preprocessing")
        scan
      case scan: FileSourceScanExec
          if isDeltaFileSourceScan(scan) &&
            !hasDeletionVectorMarkers(scan.output, scan.requiredSchema) =>
        FallbackTags.add(scan, "fallback plain Delta scan without DV preprocessing")
        scan
    }
  ...
  private def isDeltaFileSourceScan(scan: FileSourceScanExec): Boolean =
    (isDeltaLikeFileFormat(scan.relation.fileFormat) ||
  ...
  private def hasDeletionVectorMarkers(
      output: Seq[Attribute],
      requiredSchema: StructType): Boolean = {
    output.exists(_.name == DELTA_SKIP_ROW_COLUMN) ||
    requiredSchema.fieldNames.contains(DELTA_SKIP_ROW_COLUMN) ||
    output.exists(_.name == DELTA_ROW_INDEX_COLUMN) ||
    requiredSchema.fieldNames.contains(DELTA_ROW_INDEX_COLUMN) ||
    output.exists(_.name == ParquetFileFormat.ROW_INDEX_TEMPORARY_COLUMN_NAME) ||
    requiredSchema.fieldNames.contains(ParquetFileFormat.ROW_INDEX_TEMPORARY_COLUMN_NAME)
  }
